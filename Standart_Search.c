			...
			if(child!=NULL){
				...
				switch(method){
					...
					case GeneralizedAStarSearch:
                    	if(temp_node!=NULL){
							/* f(n) = (1-alpha)*g(n) + alpha*h(n) */
							float child_f = (1.0f-alpha)*child->path_cost    + alpha*child->state.h_n;
							float temp_f  = (1.0f-alpha)*temp_node->path_cost + alpha*temp_node->state.h_n;
							if(child_f < temp_f)
								Remove_Node_From_Frontier(temp_node, &frontier);
							else
							    continue;
...
{  
	...
	/* Generalized A* degerlendirme fonksiyonu: f(n) = (1-alpha)*g(n) + alpha*h(n)
	   alpha = 0  --> Uniform Cost Search (sadece g)
	   alpha = 0.5 --> Dengeli A*
	   alpha = 1  --> Greedy Search      (sadece h)                               */
	float child_f = (1.0f - alpha) * child->path_cost + alpha * child->state.h_n;
    ...
    if(Empty(*frontier)){
	    ...
	    float front_f = (1.0f - alpha) * (*frontier)->node->path_cost
	                  +         alpha  * (*frontier)->node->state.h_n;
	    if(child_f < front_f){ /* Child has the lowest f value */
	        new_queue->next = *frontier;
            *frontier = new_queue; 
        }
        else{
            for(temp_queue = *frontier; temp_queue->next != NULL; temp_queue = temp_queue->next){
                float next_f = (1.0f - alpha) * temp_queue->next->node->path_cost
                             +         alpha  * temp_queue->next->node->state.h_n;
                if(child_f < next_f){ 
                     new_queue->next = temp_queue->next;   
                     temp_queue->next = new_queue;
                     return;
                }                                              
            } /* If child has the highest f value */
            temp_queue->next = new_queue;  
            new_queue->next = NULL;                       
        } 		
	}      
