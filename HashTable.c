...
void Generate_HashTable_Key(const State *const state, unsigned char* key) 
{
	...
	/* state->city is used as a linear maze index (row * MAZE_COLS + col).
	   sprintf produces the correct decimal string without digit reversal.
	   Examples: city=0->"0", city=5->"5", city=12->"12", city=99->"99"  */
    len = sprintf((char*)key, "%d", state->row * MAZE_COLS + state->col);
